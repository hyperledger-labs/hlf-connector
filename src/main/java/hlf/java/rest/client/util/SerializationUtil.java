...
public class SerializationUtil {
  ...
  public ResponseEntity<ClientResponseModel> decodeContents(String encodedJson, boolean decodeInterior,
      boolean prettyPrint) {
    ...
    try {
      ...
      if (prettyPrint) {
        ...
        return new ResponseEntity<>(new ClientResponseModel(ErrorConstants.NO_ERROR, json), HttpStatus.OK);
      }
      return new ResponseEntity<>(new ClientResponseModel(ErrorConstants.NO_ERROR, decodedJson), HttpStatus.OK);
    } catch (Exception e) {
      log.warn("Json Deserialization Failed");
      throw new ServiceException(ErrorCode.DESERIALIZATION_FAILURE, "Json Deserialization Failed", e);
    }
  }
